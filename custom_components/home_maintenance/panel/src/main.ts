...
export class HomeMaintenancePanel extends LitElement {
    ...
    private async _handleAddTaskClick() {
        const { title, trigger_type, interval_value, interval_type, last_performed, tag, icon, label, description, area, count_entity_id, count_threshold, runtime_entity_id, runtime_threshold } = this._formData;
